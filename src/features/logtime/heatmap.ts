...
interface HeatmapColumn {
  cells: CellData[];
  /** Year of the latest non-future day in the column (the year the column "belongs" to). */
  year: number;
}
...
export function renderHeatmapCard(
  ...
  for (let w = 0; w < numWeeks; w++) {
    ...
    // A column that spans two years (e.g. Mon 29 Dec -> Sun 4 Jan) belongs to
    // the year of its latest day, like the year labels above: using the first
    // cell's year pointed the "January" label at the previous year's total.
    const colYear =
